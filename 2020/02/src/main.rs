...
fn is_password_valid_count(pw: &Password) -> bool {
    if pw.password.contains(&pw.needle.to_string()) == false {
        return false;
    }
    let mut count = 0;
...
fn is_password_valid_position(pw: &Password) -> bool {
    ...
    if chars[min] != pw.needle && chars[max] != pw.needle
        || chars[min] == pw.needle && chars[max] == pw.needle
    {
        return false;
    }
